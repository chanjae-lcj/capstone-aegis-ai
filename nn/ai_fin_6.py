...
# 실시간 모니터링 및 예측
def monitor_traffic_and_predict(model, limit=180, window=25):
    """
    ...
    while True:
        ...
        if len(traffic_history) == window:
            predicted_traffic = predict_traffic(model, list(traffic_history))
            print(f"Actual traffic: {total_traffic_bps} bps")
            print(f"Predicted traffic: {predicted_traffic} bps")

            # 임계값 초과 확인
            if predicted_traffic != 0:
                if abs(predicted_traffic / total_traffic_bps ) > limit:
